...
/**
 * Provides a configured Flink KafkaSink for sending generic String messages
 * to a specified topic.
 */
public class MoCapKafkaSink {
    private static final Logger logger = LoggerFactory.getLogger(MoCapKafkaSink.class);

    /**
     * Creates a configured KafkaSink for the specified topic and brokers.
     *
    ...
    public static KafkaSink<String> getKafkaSink(String brokers, String topic) { // Accepts arguments
        ...
        try {
            ...
            logger.info("Successfully created Kafka Sink for topic '{}'", topic);
            return kafkaSink;

        } catch (Exception e) {
            logger.error("Error creating Kafka Sink for topic '{}': ", topic, e);
            throw new RuntimeException("Error creating Kafka Sink", e);
        }
    }
