...
import org.apache.flink.api.common.functions.MapFunction; // For timestamp parsing
...
import org.example.models.SensorReading; // Your input class
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.apache.flink.api.common.typeinfo.Types; // Import for .returns()

import java.io.Serializable; // Import Serializable
import java.time.Duration;
import java.time.LocalDateTime;
import java.time.ZoneOffset; // Or specific ZoneId
import java.time.format.DateTimeFormatter;
import java.util.Map;
import java.util.HashMap;

public class ErgonomicsProcessor {
    private static final Logger logger = LoggerFactory.getLogger(ErgonomicsProcessor.class);
    private static final ObjectMapper objectMapper = new ObjectMapper();

    // --- Completed Thresholds Map ---
    // Structure: Min Normal, Max Normal, Min Yellow Buffer, Max Yellow Buffer
    ...
    // Timestamp formatter (same as before)
    // private static final DateTimeFormatter TIMESTAMP_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

    // --- Wrapper Class for Timestamp Handling ---
    public static class TimestampedSensorReading implements Serializable {
        private static final long serialVersionUID = 1L; // Good practice
        public long timestampMillis;
        public SensorReading reading;
        public TimestampedSensorReading() {}
        public TimestampedSensorReading(long ts, SensorReading r) { this.timestampMillis = ts; this.reading = r; }
        public long getTimestampMillis() { return timestampMillis; }
        public SensorReading getReading() { return reading; }
    }

    // --- Main Processing Function ---
    public static DataStream<String> processAverageAnglesForFeedback(DataStream<SensorReading> sensorStream) {

        // 1. Parse String Timestamp and Wrap the Object
        DataStream<TimestampedSensorReading> sensorStreamWithParsedTime = sensorStream
                .map(new TimestampParser())
                .filter(value -> value != null);

        // 2. Assign Timestamps and Watermarks
        DataStream<TimestampedSensorReading> sensorStreamWithTimestamps = sensorStreamWithParsedTime.assignTimestampsAndWatermarks(
                WatermarkStrategy.<TimestampedSensorReading>forBoundedOutOfOrderness(Duration.ofSeconds(5))
                        .withTimestampAssigner((event, timestamp) -> event.timestampMillis)
                        .withIdleness(Duration.ofMinutes(1))
        );

        // 3. Key the stream
        WindowedStream<TimestampedSensorReading, String, TimeWindow> windowedStream = sensorStreamWithTimestamps
                .keyBy(tsr -> tsr.reading.getThingid())
                .window(SlidingEventTimeWindows.of(Duration.ofMinutes(1), Duration.ofSeconds(10))); // Use Duration

        // 4. Calculate Averages over the Window
        ...
        // 5. Check Averages and Generate JSON Alerts
    ...
    // --- Timestamp Parsing Function (Outputs Wrapper) ---
    public static class TimestampParser implements MapFunction<SensorReading, TimestampedSensorReading> {
        private static final DateTimeFormatter FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        @Override
        public TimestampedSensorReading map(SensorReading value) throws Exception {
            try {
                ...
                return new TimestampedSensorReading(timestampMillis, value);
            } catch (Exception e) {
                logger.warn("Failed to parse timestamp string: {}. Skipping record.", value.getTimestamp(), e);
                return null;
            }
        }
    }

    // --- Helper function checkAverageAndAddAlert (Unchanged) ---
    ...
    // --- Completed Flink Windowing Functions ---
    public static class JointAngleAccumulator implements Serializable { // Ensure Accumulator is also Serializable
        private static final long serialVersionUID = 3L;
        public Map<String, Tuple2<Double, Long>> sumsAndCounts = new HashMap<>();
        public String thingId = null;

        // Completed: Accumulate all joints
        public void add(SensorReading s) {
        ...
        // Completed: Calculate all averages
    ...
    // Completed: AggregateFunction definition (Input type adjusted)
    public static class AverageAngleAggregator implements AggregateFunction<TimestampedSensorReading, JointAngleAccumulator, AverageJointAngles> {
        @Override public JointAngleAccumulator createAccumulator() { return new JointAngleAccumulator(); }
        @Override public JointAngleAccumulator add(TimestampedSensorReading value, JointAngleAccumulator acc) {
    ...
    // Completed: WindowAverageProcessor definition (Unchanged)
    public static class WindowAverageProcessor extends ProcessWindowFunction<AverageJointAngles, AverageJointAngles, String, TimeWindow> {
       @Override
        public void process(String key, Context context, Iterable<AverageJointAngles> averages, Collector<AverageJointAngles> out) {
            ...
            avg.windowEnd = context.window().getEnd();
            out.collect(avg);
        }
    }
